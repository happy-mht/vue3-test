...
export default {
  ...
  data() {
    const checkName = async (rule, value, cb) => {
      try {
        const res = await api.checkSetName({ name: value, id: this.id || 0 })
        if (res) cb('名称已经存在')
        else cb()
      } catch (e) {
        cb(e.message)
    ...
    return {
      ...
      rules: {
        name: [
          ...
          { message: '名称已经存在', validator: checkName, trigger: 'blur' }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    async getDetail() {
      try {
        this.loading = true
        const res = await api.getDetail({ shopGoodsSetId: this.id || 0 })
        if (this.id) {
    ...
    async handleOk() {
      try {
        ...
        const request = this.id ? api.updateGoodsSet : api.addGoodsSet
        const params = {
          platform: this.setForm.platform,
          name: this.setForm.name,
          numIidList: this.numIidList
        }
